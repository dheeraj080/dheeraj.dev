...
export default function Work({ onProjectClick }: { onProjectClick?: (slug: string) => void }) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Calculate exact progress points for each card to snap to
      const snapPoints = [];
      for (let i = 0; i < numCards; i++) {
        snapPoints.push(startDwell + (i / (numCards - 1)) * moveFraction);
      }
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          height: "8rem",
          background:
            "linear-gradient(to bottom, rgba(15,15,17,1) 0%, rgba(15,15,17,0.7) 60%, transparent 100%)",
          zIndex: 10,
          pointerEvents: "none",
