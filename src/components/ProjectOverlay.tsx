...
function PortfolioVisuals() {
  ...
  return (
    <>
      <div style={{ width: "100%", aspectRatio: "16/9", position: "relative", overflow: "hidden", background: "#0F0F11" }}>
        ...
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(90deg, #0F0F11 0%, transparent 10%, transparent 90%, #0F0F11 100%)" }} />
        <div style={{ position: "absolute", bottom: "2rem", left: "2rem" }}>
          <p style={{ fontFamily: "var(--font-spacemono)", fontSize: "0.625rem", color: "#F97316", letterSpacing: "0.2em", opacity: 0.6 }}>CANVAS 2D WAVEFORM</p>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }}>
        <div style={{ aspectRatio: "1", position: "relative", overflow: "hidden", background: "#0F0F11" }}>
          <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "1.5rem" }}>
            <div style={{ fontFamily: "var(--font-bebas)", fontSize: "clamp(4rem, 10vw, 9rem)", color: "#F3F4F6", lineHeight: 0.85, opacity: 0.06, position: "absolute", top: "50%", left: "-0.02em", transform: "translateY(-50%)", whiteSpace: "nowrap" }}>DHEERAJ</div>
            <p style={{ fontFamily: "var(--font-spacemono)", fontSize: "0.625rem", color: "#F97316", letterSpacing: "0.2em", opacity: 0.6, position: "relative" }}>BEBAS NEUE / DISPLAY</p>
          </div>
        </div>
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", padding: "2rem", gap: "0.5rem" }}>
            {["NEXT.JS", "GSAP", "LENIS", "CANVAS 2D", "TAILWIND V4"].map((tech, i) => (
              <div key={tech} style={{ display: "flex", alignItems: "center", gap: "0.75rem", opacity: 0.3 + i * 0.1 }}>
                ...
                <span style={{ fontFamily: "var(--font-spacemono)", fontSize: "0.6rem", color: "#F3F4F6", letterSpacing: "0.1em" }}>{tech}</span>
              </div>
            ))}
          </div>
        </div>
...
function ProjectContent({
  ...
  return (
    <div style={{ opacity: animateIn ? 1 : 0, transition: "opacity 0.4s ease" }}>
      {/* ── Hero ─────────────────────────────────────────────────── */}
      <div style={{ position: "relative", height: "100vh", background: "#0F0F11", overflow: "hidden" }}>
        {project.bgGif && (
          <Image
            src={project.bgGif}
            alt=""
        ...
        {project.bgGif && (
          <div style={{
            position: "absolute",
            inset: 0,
            background: "linear-gradient(180deg, transparent 15%, #0F0F1140 45%, #0F0F11cc 72%, #0F0F11 100%)",
            pointerEvents: "none",
            zIndex: 2,
          }} />
        )}
        ...
        <div style={{ position: "absolute", top: "3rem", right: "clamp(2rem, 6vw, 8rem)", textAlign: "right", zIndex: 3 }}>
          ...
          <p style={{ fontFamily: "var(--font-spacemono)", fontSize: "0.625rem", letterSpacing: "0.12em", color: "#F3F4F6", opacity: 0.3, marginTop: "0.25rem" }}>{project.role}</p>
        </div>

        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, zIndex: 3, padding: "4rem clamp(2rem, 6vw, 8rem)" }}>
          <div style={{ overflow: "hidden" }}>
            <h1 style={{ fontFamily: "var(--font-bebas)", fontSize: "clamp(16vw, 22vw, 26vw)", lineHeight: 0.85, color: "#F3F4F6", marginLeft: "-0.03em" }}>
              {project.title}
            </h1>
          </div>
      ...
      {/* ── Content ──────────────────────────────────────────────── */}
      <div ref={contentRef}>
        <div data-reveal style={{ padding: "clamp(5rem, 10vw, 12rem) clamp(2rem, 6vw, 8rem)", display: "grid", gridTemplateColumns: "minmax(0, 2fr) minmax(0, 1fr)", gap: "clamp(3rem, 6vw, 8rem)", borderBottom: "1px solid #1F2937" }} className="project-story-grid">
          <div>
            <p className="section-label" style={{ marginBottom: "2.5rem" }}>THE STORY</p>
            {project.story.map((para, i) => (
              <p key={i} style={{ fontFamily: "var(--font-spacemono)", fontSize: "clamp(0.875rem, 1.2vw, 1.1rem)", lineHeight: 1.9, color: "#F3F4F6", opacity: 0.65, marginBottom: i < project.story.length - 1 ? "1.5rem" : 0 }}>
                {para}
              </p>
            ))}
          </div>
          <div style={{ paddingTop: "2.5rem" }}>
            <div style={{ borderTop: "1px solid #1F2937", paddingTop: "2rem", marginBottom: "2.5rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                {project.stack.map((tech) => (
                  <div key={tech} style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                    ...
                    <span style={{ fontFamily: "var(--font-spacemono)", fontSize: "0.8rem", color: "#F3F4F6", opacity: 0.55 }}>{tech}</span>
                  </div>
                ))}
              </div>
            </div>
            <div style={{ borderTop: "1px solid #1F2937", paddingTop: "2rem", marginBottom: "2rem" }}>
              <p className="section-label" style={{ marginBottom: "0.75rem" }}>ROLE</p>
              <p style={{ fontFamily: "var(--font-spacemono)", fontSize: "0.8rem", color: "#F3F4F6", opacity: 0.45 }}>{project.role}</p>
            </div>
            <div style={{ borderTop: "1px solid #1F2937", paddingTop: "2rem" }}>
              <p className="section-label" style={{ marginBottom: "0.75rem" }}>YEAR</p>
              <p style={{ fontFamily: "var(--font-spacemono)", fontSize: "0.8rem", color: "#F3F4F6", opacity: 0.45 }}>{project.year}</p>
            </div>
            {project.externalUrl && (
              <div style={{ borderTop: "1px solid #1F2937", paddingTop: "2rem", marginTop: "2rem" }}>
                <a
        ...
        {/* ── Visuals ────────────────────────────────────────────── */}
        <div data-reveal style={{ padding: "clamp(4rem, 8vw, 10rem) 0", borderBottom: "1px solid #1F2937" }}>
              ...
              <div style={{ padding: "0 clamp(2rem, 6vw, 8rem)" }}>
                <div style={{
                  ...
                  <div style={{
                    position: "absolute",
                    inset: 0,
                    background: `linear-gradient(180deg, transparent 80%, #0F0F1140 100%)`,
                    pointerEvents: "none",
                  }} />
                </div>
              </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                      {project.stack.map((tech, i) => (
                        <div key={tech} style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                          ...
                          <span style={{ fontFamily: "var(--font-spacemono)", fontSize: "0.8rem", color: "#F3F4F6", letterSpacing: "0.08em", opacity: 0.5 + i * 0.1 }}>{tech}</span>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        <div data-reveal style={{
          ...
          <div>
            {prev ? (
              <button onClick={() => onNavigate(prev.slug)} style={navBtnStyle}>
                ...
                <p style={{ fontFamily: "var(--font-bebas)", fontSize: "clamp(2.5rem, 5vw, 6rem)", color: "#F3F4F6", lineHeight: 0.88, opacity: 0.45, transition: "opacity 0.2s" }}
          ...
          <div style={{ width: "1px", background: "#1F2937", alignSelf: "stretch" }} />
...
export default function ProjectOverlay({
  ...
  return (
    ...
    >
      <div style={{ position: "sticky", top: 0, zIndex: 10, background: "#0F0F11", padding: "1.25rem clamp(2rem, 6vw, 8rem)", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #1F2937" }}>
        <span className="section-label">/ PROJECT VIEW</span>
        <button
          onClick={doClose}
          style={{ background: "none", border: "none", cursor: "none", fontFamily: "var(--font-spacemono)", fontSize: "0.625rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "#F97316", padding: 0 }}
        >
          ✕ CLOSE
        </button>
      </div>
