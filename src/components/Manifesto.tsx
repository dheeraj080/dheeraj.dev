...
export default function Manifesto({ onWaveformUpdate }: ManifestoProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      if (reduced) {
        gsap.set([line1, line2, line3], { color: "#F3F4F6", opacity: 1 });
        return;
      }
      ...
      gsap.set(allWords, { color: "#1F2937", opacity: 0.15 });
      ...
      // At very end: all words get a subtle signal-red tint on "SYSTEMS."
      const lastWords = split3.words;
      tl.to(lastWords, { color: "#F97316", ease: "none" }, 0.88);
      // Then snap back to ivory (the "systems" word burns red then settles)
      tl.to(lastWords, { color: "#F3F4F6", ease: "none" }, 0.95);
      ...
      <div ref={innerRef} className="manifesto-inner">
        ...
        */}
        ...
        >
          <p
            className="text-mono"
            style={{ fontSize: "0.625rem", color: "#F3F4F6", opacity: 0.3, letterSpacing: "0.15em" }}
          >
            dheerajkamble / 2026
          </p>
        </div>
